...
import cv2
from Ben import SimplePongEnv 
import os

from sympy import false

os.makedirs(r"C:\Users\USER\AppData\Roaming\JetBrains\PyCharm2025.2\scratches\data", exist_ok=True)
...
MODEL_PATH = r"C:\Users\USER\AppData\Roaming\JetBrains\PyCharm2025.2\scratches\data"
SAVE_MODEL_INTERVAL = 100  # 每 N 個 episode 保存一次模型
TRAIN_MODEL = True  # 是否訓練

LOAD_MODEL_FROM_FILE = False # 是否從檔案載入模型
LOAD_FILE_EPISODE = 900  # 從第幾回合載入模型

# -----------------------------------
# 訓練超參數
BATCH_SIZE = 32  # minibatch 大小
MAX_EPISODE = 100000  # 最大回合數
MAX_STEP = 100000  # 單回合最大步數

MAX_MEMORY_LEN = 50000  # replay memory 最大長度
MIN_MEMORY_LEN = 5000  # 訓練前最小 memory 長度

GAMMA = 0.97  # 折扣率
ALPHA = 0.0001  # 學習率
EPSILON_DECAY = 0.995  # epsilon 衰減率

RENDER_GAME_WINDOW = True  # 是否顯示遊戲畫面
...
# -----------------------------------
# 初始化 best model
best_avg_reward = -float('inf')  # 目前最佳平均 reward
last_100_ep_reward = deque(maxlen=100)  # 保存最近 100 回合 reward
    ...
    for episode in range(startEpisode, MAX_EPISODE):
        ...
        for step in range(MAX_STEP):
           ...
           # Select and perform an action
                ...
                # Stack frames: [new, old0, old1, old2]
            ...
            if done:  # Episode completed
                ...
                if SAVE_MODELS and episode % SAVE_MODEL_INTERVAL == 0:  # Save model as file
                    weightsPath = os.path.join(MODEL_PATH, f"{episode}.pkl")
                    epsilonPath = os.path.join(MODEL_PATH, f"{episode}.json")
                    outputPath = os.path.join(MODEL_PATH, "out.txt")
