...
1) Ben_DQN.py 中的目標 Q 值（target Q）：
   ...
   - next_states_target_q_value 的來源：
     ...
     - 程式對應位置：Agent.train() 中的 next_states_target_q_value 計算與 expected_q_value 行。
   - 因此，形式上等同於：
...
2) A（Advantage）是否等於 reward？Ben_DQN.py 中每次 reward 為多少？
   ...
   - Ben_DQN.py 中的 reward 是由環境（SimplePongEnv）在 environment.step(action) 中回傳，程式並未將 A 與環境的 reward 混為同一個值。
   - 依本專案與典型 Atari Pong 設計，環境回傳的即時 reward 常見集合為 {-1, 0, +1}，程式文件中也提及 r ∈ {-1,0,+1}，所以每步 reward 很可能為 -1、0 或 +1，視環境情況（得分、失分或中性步驟）而定。
