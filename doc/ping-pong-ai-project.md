...
- $Q_{target}(S,A) = R + \gamma \max_{A'} Q_{\theta'}(S', A')$，是使用 Bellman 方程計算的目標值。
  - $R$: 執行動作 A 後獲得的即時獎勵。
  - $\gamma$: 折扣因子 (Discount Factor)，通常設為 0.99，用於平衡短期與長期獎勵。
  - $S', A'$: 下一個狀態與可能的動作。
  - $\theta'$: 目標網絡 (Target Network) 的參數，定期從主網絡 $\theta$ 同步，以增加訓練穩定性。
...
### 4.2.1 `game.py` (遊戲引擎)
- **`class Game:`**
  - `__init__(self)`: 初始化遊戲窗口、球、球拍。
  - `reset(self)`: 重置遊戲狀態，返回初始狀態 S₀。
  - `step(self, action)`: 執行一個動作，更新物理狀態，返回 (S', R, done)。
  - `_update_physics(self)`: 更新球的位置、速度。
  - `_check_collision(self)`: 檢查碰撞 (邊界、球拍)。
  - `_calculate_reward(self)`: 計算獎勵。
  - `render(self)`: 渲染遊戲畫面。

### 4.2.2 `agent.py` (DQN 代理)
- **`class DQNAgent:`**
  - `__init__(self, state_dim, action_dim)`: 初始化主網絡、目標網絡、緩衝區、優化器。
  - `get_action(self, state, epsilon)`: 根據 ε-greedy 策略選擇動作。
  - `store_experience(self, state, action, reward, next_state, done)`: 存儲經驗到緩衝區。
  - `train(self, batch_size)`: 從緩衝區採樣並訓練網絡。
  - `update_target_network(self)`: 同步目標網絡權重。
  - `save_model(self, path)`: 保存模型。
  - `load_model(self, path)`: 加載模型。

### 4.2.3 `model.py` (神經網絡)
- **`create_dqn_model(input_shape, num_actions)`**:
  - 使用 TensorFlow/Keras 建立一個符合 3.8 節設計的序列模型。
  - 返回未編譯的模型實例。
...
| 任務 (Task)                | 負責人 (Owner) | 開始日期 (Start) | 結束日期 (End) | 狀態 (Status) |
|----------------------------|----------------|------------------|----------------|---------------|
| **階段一：規劃與設計**     | 團隊           | 2025-11-01       | 2025-11-07       | ✅ 完成       |
| 需求分析與文件撰寫         | 產品經理       | 2025-11-01       | 2025-11-03       | ✅ 完成       |
| 系統分析與架構設計         | 架構師         | 2025-11-04       | 2025-11-07       | ✅ 完成       |
| **階段二：開發與實現**     | 團隊           | 2025-11-08       | 2025-11-22       | ⏳ 進行中     |
| 遊戲引擎開發 (`game.py`)   | 開發者A        | 2025-11-08       | 2025-11-14       | ✅ 完成       |
| DQN模型建立 (`model.py`)   | 開發者B        | 2025-11-11       | 2025-11-15       | ✅ 完成       |
| AI代理開發 (`agent.py`)    | 開發者B        | 2025-11-15       | 2025-11-20       | ⏳ 進行中     |
| 訓練腳本整合 (`main.py`)   | 開發者A        | 2025-11-18       | 2025-11-22       | ⏳ 進行中     |
| **階段三：測試與驗證**     | 測試團隊       | 2025-11-23       | 2025-11-28       | 📋 待辦      |
| 單元與集成測試             | 開發者/測試者  | 2025-11-23       | 2025-11-25       | 📋 待辦      |
| 模型訓練與性能調優         | 開發者B        | 2025-11-24       | 2025-11-28       | 📋 待辦      |
| **階段四：交付與部署**     | 團隊           | 2025-11-29       | 2025-11-30       | 📋 待辦      |
| 文件整理與專案總結         | 全體           | 2025-11-29       | 2025-11-30       | 📋 待辦      |
